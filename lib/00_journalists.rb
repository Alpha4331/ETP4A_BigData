...
nb_journalistes = handles.size

# Combien d'handle contiennent un numéro ?
nb_handles_numero = handles.count { |handle| handle =~ /\d/ }

# Combien d'handle contiennent les 4 lettres du prénom "aude" à la suite ?
nb_handles_aude = handles.count { |handle| handle =~ /aude/i }

# Combien commencent par une majuscule (première lettre juste après le @) ?
nb_handles_majuscule_debut = handles.count { |handle| handle =~ /^@[A-Z]/ }

# Combien contiennent au moins une majuscule ?
nb_handles_avec_majuscule = handles.count { |handle| handle =~ /[A-Z]/ }

# Combien y a-t-il de underscore _ dans tous les pseudos confondus ?
nb_underscore_total = handles.join.count('_')

# Trie la liste de handle par ordre alphabétique.
handles_tries = handles.sort

# Quels sont les 50 handles les plus courts de ce array ?
handles_plus_courts = handles.sort_by { |handle| handle.length }.first(50)

# Quelle est la position dans l'array de la personne @epenser ?
position_epenser = handles.index("@epenser") + 1

# Affichage des résultats
puts "Nombre de journalistes : #{nb_journalistes}"
puts "Nombre d'handles contenant un numéro : #{nb_handles_numero}"
puts "Nombre d'handles contenant 'aude' à la suite : #{nb_handles_aude}"
puts "Nombre d'handles commençant par une majuscule : #{nb_handles_majuscule_debut}"
puts "Nombre d'handles contenant au moins une majuscule : #{nb_handles_avec_majuscule}"
puts "Nombre total d'underscore _ dans les pseudos : #{nb_underscore_total}"
puts "Liste des handles triés par ordre alphabétique :"
puts handles_tries
puts "Les 50 handles les plus courts :"
puts handles_plus_courts
puts "Position de @epenser dans l'array : #{position_epenser}"
